  ...
  return (
    <div className="w-64 h-[calc(100vh-64px)] bg-white dark:bg-gray-800 border-r border-gray-200 dark:border-gray-700 flex flex-col py-6 sticky top-16">
      <div className="px-4 mb-8 flex flex-col items-center">
        <img src="/profermaco-white-logo.avif" alt="Profermarco Logo" className="h-16 w-auto mb-2 object-contain" />
        ...
        <div className="flex flex-col gap-2 px-2">
          {menuItems.map((item) => (
            <Link
              key={item.key}
              href={item.href}
              className={`flex items-center gap-3 px-4 py-3 rounded-lg transition-colors font-medium ${currentKey === item.key
